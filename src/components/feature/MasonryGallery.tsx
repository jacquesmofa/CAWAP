...
const MasonryGallery: React.FC<MasonryGalleryProps> = ({ photos = [], videos = [], onPhotoClick }) => {
  ...
  // Filter out failed images and videos for display count
  const visiblePhotos = safePhotos.filter((_, idx) => !failedImages.has(idx));
  const visibleVideos = safeVideos.filter((_, idx) => !failedVideos.has(idx));
  const isEmpty = visiblePhotos.length === 0 && visibleVideos.length === 0;

  // Show placeholder only if ALL images have been checked and failed
  const allPhotosChecked = safePhotos.length > 0 && loadedImages.size + failedImages.size >= safePhotos.length;
  const allVideosChecked = safeVideos.length > 0 && loadedVideos.size + failedVideos.size >= safeVideos.length;
  const shouldShowPlaceholder = isEmpty && allPhotosChecked && allVideosChecked;
  ...
  return (
    <div className="w-full">
      ...
      {safeVideos.length > 0 && (
        <div className="mb-12">
          <h3 className="text-2xl font-bold text-gray-800 mb-6 flex items-center">
            <i className="ri-video-line mr-3 text-[#3c1053]"></i>
            Videos {visibleVideos.length > 0 && `(${visibleVideos.length})`}
          </h3>
      ...
      {safePhotos.length > 0 && (
        <>
          <h3 className="text-2xl font-bold text-gray-800 mb-6 flex items-center">
            <i className="ri-image-line mr-3 text-[#3c1053]"></i>
...
const LazyImage: React.FC<LazyImageProps> = ({
  ...
  // Hide completely if failed - no placeholder
  if (hasFailed) {
    return null;
  }
  ...
  return (
    <div
      ...
      onClick={() => imageLoaded && onClick?.(index)}
    >
        ...
        {!isLoaded ? (
          ...
          <>
            ...
            <img
              ...
              onError={() => {
                console.warn(`Media not found: ${photoUrl}`);
                onError(index);
              }}
            />
            {imageLoaded && (
